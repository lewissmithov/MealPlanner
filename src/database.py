...
class DBHandler():
    ...
    def add_meal(meal):
        """
        Docstring: This is a method to add a single meal to
        the sqlite db
        """
        query = f"INSERT INTO meals (name, healthScore, ketoPotential, gfPotential) VALUES (?, ?, ?, ?)"

        con = sqlite3.connect("meals.db")
        cur = con.cursor()
        cur.execute(query, (meal.get_name(), meal.get_health_score(), meal.get_keto_potential(), meal.get_gf_potential()))
        con.commit()
        con.close()
